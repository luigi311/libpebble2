...
def unhex(string: str) -> bytes:
    """Return raw bytes from a hex string (no spaces needed)."""
...
def test_pascal_list_count_serialise(packet):
    ...
    assert field.value_to_bytes(packet, [Foo(foo=1)]) == b"\x01\x01"

    field.prepare(packet, [Foo(foo=1), Foo(foo=2)])
    assert packet.count == 2
    assert field.value_to_bytes(packet, [Foo(foo=1), Foo(foo=2)]) == b"\x01\x01\x01\x02"
